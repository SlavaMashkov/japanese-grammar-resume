...
def build():
    ...
    for n in [
        ...
        jp("Polite forms also work: です → でして, ます → まして."),
    ]:
        te_notes.append(Paragraph(f"- {n}", note_s))
    ...
    for n in [
        ...
        jp("For nouns / na-adj: add だ before から (友達だからプレゼントが来た)."),
        jp("ので is softer and more polite than から. For nouns / na-adj: add な before ので."),
        jp("  私は学生なので、お金がないんです。 — Because I'm a student, I have no money."),
        jp("  ちょっと忙しいので、そろそろ失礼します。 — A little busy, so I'll be leaving soon."),
        jp("ので can shorten to んで in speech (時間がなかったんでパーティーに行かなかった)."),
        jp("Either the reason or the result can be omitted if clear from context."),
    ]:
        kara_notes.append(Paragraph(f"- {n}", note_s))
    ...
    for n in [
        ...
        jp("  学生なのに、彼女は勉強しない。 — Despite being a student, she does not study."),
        jp("For nouns / na-adj: add な before のに (same as ので)."),
    ]:
        noni_notes.append(Paragraph(f"- {n}", note_s))
    ...
    for n in [
        ...
        jp("  友達に聞いたけど、知らなかった。 — Asked a friend but (he) didn't know."),
        jp("For nouns / na-adj: add だ before けど (今日は暇だけど、明日は忙しい)."),
        jp("More formal: けど → けれど → けれども."),
        jp("Can also just connect two topics (no contradiction):"),
        jp(
            "  デパートに行きましたが、いい物がたくさんありました。 — Went to dept store and there was a lot of good stuff."
        ),
    ]:
        ga_notes.append(Paragraph(f"- {n}", note_s))
    ...
    for n in [
        ...
        jp("  先生だし、年上だし… — He's the teacher, and older..."),
        jp("だ required for non-conjugated nouns / na-adj before し (先生だし, 好きだし)."),
        jp("し implies there may be other reasons beyond those listed."),
    ]:
        shi_notes.append(Paragraph(f"- {n}", note_s))
    ...
    for n in [
        ...
        tari_notes.append(Paragraph(f"- {n}", note_s))
